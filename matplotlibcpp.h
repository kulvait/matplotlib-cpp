...
#include <cstdio> //Standard I/0 like printf, fprintf, ...
#include <functional>
#include <iostream>
#include <map>
#include <numeric>
...
namespace detail {
    ...
    struct _interpreter
    {
        ...
        /* For now, _interpreter is implemented as a singleton since its currently not possible to
           have multiple independent embedded python interpreters without patching the python source
           code or starting a separate process for each. [1] Furthermore, many python objects expect
           that they are destructed in the same thread as they were constructed. [2] So for advanced
           usage, a `kill()` function is provided so that library users can manually ensure that the
...
namespace detail {
    ...
    struct plot_impl<std::false_type>
    {
        ...
        bool operator()(const IterableX& x, const IterableY& y, const std::string& format)
        {
            ...
            PyObject* res = PyObject_CallObject(detail::_interpreter::get().s_python_function_plot,
                                                plot_args);
